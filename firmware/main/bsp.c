...
static lv_font_t *s_cjk_14, *s_cjk_16, *s_cjk_20, *s_cjk_24, *s_cjk_28, *s_cjk_32, *s_cjk_36;
...
static void init_lvgl(void)
{
    ...
    const lvgl_port_display_cfg_t disp_cfg = {
        ...
        .buffer_size = LCD_H_RES * 80,    /* 80 行 单缓冲 → ~6 flush/帧 */
        .double_buffer = false,            /* 避免双缓冲 DRAM 碎片化 */
        .hres = LCD_H_RES,
        .vres = LCD_V_RES,
        .monochrome = false,
    ...
    /* TinyTTF: copy font data from flash to PSRAM, then create LVGL fonts */
    {
        size_t buf_sz;
        uint8_t *buf;

        /* Emoji */
        buf_sz = (size_t)emoji_ttf_len;
        buf = heap_caps_malloc(buf_sz, MALLOC_CAP_SPIRAM);
        if (buf) {
            memcpy(buf, emoji_ttf_data, buf_sz);
            s_emoji_font = lv_tiny_ttf_create_data(buf, buf_sz, 24);
        }
        if (s_emoji_font) {
        ...
        /* CJK */
        if (cjk_ttf_len > 0) {
            buf_sz = (size_t)cjk_ttf_len;
            buf = heap_caps_malloc(buf_sz, MALLOC_CAP_SPIRAM);
            if (buf) {
                memcpy(buf, cjk_ttf_data, buf_sz);
                const int sizes[] = {14, 16, 20, 24, 28, 32, 36};
                lv_font_t **slots[] = {&s_cjk_14, &s_cjk_16, &s_cjk_20, &s_cjk_24, &s_cjk_28, &s_cjk_32, &s_cjk_36};
                for (int i = 0; i < 7; i++) {
                    *slots[i] = lv_tiny_ttf_create_data(buf, buf_sz, sizes[i]);
                    if (*slots[i])
                        ESP_LOGI(TAG, "CJK font %dpx loaded", sizes[i]);
                }
            } else {
                ESP_LOGW(TAG, "CJK font PSRAM alloc failed (%d bytes)", (int)buf_sz);
            }
        } else {
            ESP_LOGW(TAG, "CJK font not available (0 bytes)");
        }
    }
...
lv_font_t *bsp_cjk_font(int size)
{
    ...
    case 32: return s_cjk_32;
    case 36: return s_cjk_36;
    default: return NULL;
    }
}
